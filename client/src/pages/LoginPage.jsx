...

        {
          currState === "Sign up" && isDataSubmitted && (
            <textarea onChange={(e) => setBio(e.target.value)}
              value={bio} rows={4} className='p-2 border border-gray-500 rounded-md
        ...
        <div className='flex flex-col gap-2'>
          {currState === "Sign up" ? (
            ...
            <p className='text-sm text-gray-600'>Create an account. <span
              className='font-medium text-violet-500 cursor-pointer' onClick={() => { setCurrState("Sign up") }}>Click here
            </span></p>
          )}
        </div>
      </form>
